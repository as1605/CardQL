...
1. Use **IMAP** to search for each email pattern and fetch new statements (implemented: `cardql imap fetch`, config in `card_rules.json`, 5 workers per rule)
2. Download the PDFs neatly into a folder under **`data/raw-pdfs/`** (gitignored) — done as part of `cardql imap fetch`
